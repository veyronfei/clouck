...
function filterVersions(accountId, resourceType) {
	...
	oTable.fnReloadAjax('/app/rest/dataTable/accounts/' + accountId + '/ec2/versions/' + resourceType + query);
}
...
function clearFilter(accountId, resourceType) {
	...
	oTable.fnReloadAjax('/app/rest/dataTable/accounts/' + accountId + '/ec2/versions/' + resourceType);
}
...
function changeRegion(accountId, regionEndpoint) {
	if (regionEndpoint) {
		window.location.href = "/app/accounts/" + accountId + "/ec2/overview?region=" + regionEndpoint;
	} else {
		window.location.href = "/app/accounts/" + accountId + "/ec2/overview";
	}
}

function deleteAccount(accountId) {
	$.ajax({
		url: '/app/rest/accounts/' + accountId,
		type: 'DELETE',
...
function showAllRegions(variableName, millisecond, currentAccountId) {
    oTable.fnReloadAjax('/app/rest/dataTable/' + variableName + '/' + millisecond + '/accounts/' + currentAccountId);
}

function showCurrentRegion(ec2VersionId) {
    oTable.fnReloadAjax('/app/rest/dataTable/' + ec2VersionId);
}

function createChart(variableName, timeStamp, currentAccountId) {
    $.getJSON('/app/rest/highCharts/' + variableName + '/' + timeStamp + '/accounts/' + currentAccountId, function(data) {
...
function loadOverviewChart(chartId, variableName, currentAccountId, millis, regionEndpoint) {
	var url = '/app/rest/chart-data/' + variableName + '/' + millis + '?account-id=' + currentAccountId;
	if (regionEndpoint) {
...
//            click : function() {location.href = '/app/ec2/' + variableName + '/' + millis + '?account-id=' + currentAccountId + '&region=' + regionEndpoint}
//        };
